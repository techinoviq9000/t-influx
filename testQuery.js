...
const middleString = (item) => {
  return  `${item.name}: insert_data_table_one(
    object: {
      ...
      analysis: $analysis_${item.id}
      applicant_id: $applicant_id
    },
    on_conflict: {constraint: data_table_field_id_applicant_id_key, update_columns: [value, analysis], where: {field_id: {_eq:  $field_${item.id}}}}
  ) {
    id
  } `
  }
  const generateQuery = (data) => {
    let startString = `mutation MyMutation(`
      ...
      let middlestring = ""
      data.map(item => {
        firstString+=`$${item.name}: String, $field_${item.id}: Int! $analysis_${item.id}: json
        `
